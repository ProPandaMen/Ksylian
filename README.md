...
Актуальная версия проекта: `0.6.57`. Источник версии — [VERSION](./VERSION), release tags используют формат `v0.6.57`.
...
Ksylian обновляется через release tags вида `v0.6.57`. Текущая версия хранится в [VERSION](./VERSION), а `deploy/.env` получает `KSYLIAN_BUILD_VERSION` и `KSYLIAN_BUILD_SHA` во время обновления.
...
```bash
cd /opt/ksylian
sudo deploy/scripts/update-ksylian.sh v0.6.57
```
